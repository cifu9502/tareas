...
integrate(sq, 0, 1)
```
Como se puede ver los resultados son bastante cercanos, sin embargo, ya que solo estamos usando 2000 números aleatorios, varía bastante cada vez que  se corre el programa. Una forma de mejorarlo sería utilizando más números aleatorios(lo cual no se realizó gráficamente ya que no se visualizaría bien la gráfica). Sin embargo este resultado más exacto (realizado con 10000 puntos) se muestra a continuación:
```{r, echo=FALSE}
int1(sq, 0, 1)
```
...
intInfinity(func,2)
```
Comparando nuestro resultado con el de la función integrate(que se encuentra a continuación), vemos que es bastante cercano pero todavía falta presición. El método utilizado fue básicamente integrar de a intervalos utilizando la función creada de (0-100), (100-200) , (200-300) ... hasta que los términos añadidos sean obsoletos.
```{r, echo=FALSE}
